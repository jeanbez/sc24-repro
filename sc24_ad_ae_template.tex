...
\artexpl{
List the computational artifacts you generated, while reproducing the original work, along with their respective DOIs. Note that all computational artifacts can be archived under a single DOI. If that is the case, use the same DOI for all artifacts and explicitly state how it is organized.
}
...
highlight their relation to the reproducibility challenge, 
i.e., point to the elements in the paper that are reproducible 
by each artifact, e.g., which figures or tables were generated 
with the artifact.
}
...
\begin{tabular}{rl}
\toprule
Artifact ID  &  Paper Elements \\
\midrule
$A_1$   &  Table (Figure 2.f) \\
\midrule
$A_2$   &  Figure 4.a \\
\midrule
$A_3$   &  Figure 6 \\
...
\artexpl{
Briefly explain the relationship between the artifact and challenge.
}

\artexp

\artexpl{
Provide a higher level description of what outcome to expect from the corresponding experiments. 
}
...
\begin{itemize}
    ...
    \item What were the challenges reproducing the work?
    \item What results did you obtain? Explain how the expected results from the experiment workflow correlate with the contributions stated in the article.
    \item Were the results affected by the change in environment?
    \item Can we reach equivalent conclusions or interpretations? If yes, which ones? If no, what are the differences and what could explain them?
\end{itemize}
}
